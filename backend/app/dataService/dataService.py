...
class DataService(object):
    def __init__(self):
        ...
        self.GV = GV
        self.openai_key = GV.azure_openai_key if GV.azure_openai_key else GV.openai_key
        self.utils = utils
    ...
    def _load_vectorstores_(self, load_flag=True):
        ...
        for pdf_file in os.listdir(data_folder):
            if pdf_file.endswith(".pdf"):
                ...
                if load_flag: 
                    embedding_model = AzureOpenAIEmbeddings(azure_endpoint=GV.azure_openai_endpoint,
                                                           azure_deployment=GV.azure_openai_deployment,
                                                           api_version=GV.azure_openai_version,
                                                           api_key=GV.azure_openai_key) if GV.azure_openai_key else OpenAIEmbeddings(openai_api_key=GV.openai_key)
                    ...
                    key = GV.azure_openai_key if GV.azure_openai_key else GV.openai_key
                    vectorstore, docstore = utils.build_local_document_vector_store(all_text, key)
